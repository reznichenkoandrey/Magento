...
class RequestSalesDocumentTest extends TestCase
{
    ...
    {
        // `ContextExtensionInterface` is generated by `setup:di:compile` from the merged
        // `extension_attributes.xml`, and its method set depends on which modules are installed.
        // Without a compile the unit framework still supplies the interface — but as an empty
        // marker, and Magento's own docblock on `ExtensionAttributesInterfaceGenerator` says so:
        // "For unit tests, these are just empty type definitions. You should use integration tests
        // if you want to see the real types."
        //
        // PHPUnit will not configure a method the interface does not declare, so these mocks work
        // on a compiled installation and fail on a clean checkout. Skipping says which of the two
        // you are on. A hand-written double is not the answer: this stand compiles six methods onto
        // this interface, and another installation compiles a different set.
        if (!method_exists(ContextExtensionInterface::class, 'getIsCustomer')) {
            self::markTestSkipped(
                'Needs a compiled installation: run bin/magento setup:di:compile so that '
                . 'ContextExtensionInterface carries its extension attributes.'
            );
        }
